...
```{r overallpositivity, echo=FALSE}
...
tbl_combi %>% kbl(caption ="<center><strong>Overall Positivity by County</strong></center>") %>%
  kable_classic_2("hover", full_width = F,html_font = "Cambria")
```

### Positive Precision by County for Different Risk Factor Combinations
...
ggplot(combiHighRisk,aes(x=reorder(RiskOutcome,-prec_pos),y=prec_pos,fill=RiskOutcome))+
...
scale_y_continuous(labels = scales::percent,limit=c(0,0.80))
...
posit_age$groupCounty <- factor(posit_age$county,      # Reordering group factor levels
                         levels = c("Siaya", "Homabay", "Nairobi", "Machakos"))
