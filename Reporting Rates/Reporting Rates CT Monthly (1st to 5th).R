...
rr<-read_excel("ReportingRates_CT_Raw.xlsx")

#Declare From and To Dates
FromDate=as.Date("2021-05-01",origin = "1899-12-30")
#ToDate = as.Date("2021-05-31",origin = "1899-12-30")
ToDate=ceiling_date(ymd(as.Date(FromDate)) %m+% months(0), "month")+days(4)

#Replace NULL with na in the entire dataset
rr<-na_if(rr,"NULL")

#Convert Upload Dates from Numeric to Dates
...
rr <-rr %>%
  group_by(DisplayMFL,UploadDate) %>%
...
#Number of Duplicates in CT_Recency Those that reported in both May and between 1st and 5th June
...
write.xlsx(rt,file="CTRecency.xlsx")
